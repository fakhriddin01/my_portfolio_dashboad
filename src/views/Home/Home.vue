...
import { useComment } from '../../service/comment';
...
const toggleModal = () => {
    if(modal.value){
        ...
        contactInfo.isAllowed=''
    }
    modal.value = !modal.value
}

const updateList = () => {
    useComment.list().then((res)=>{
...
const addContact=(evet)=>{
    evet.preventDefault();
    const contact = {
        isAllowed: contactInfo.isAllowed,
    }

    useComment.create(contact).then((res)=>{
        if(res.status == 201){
...
const modifyContact=(event)=>{
    ...
    useComment.updateStatus(id, contact).then((res)=>{
        if(res.status == 200){
...
const updateContact = (id)=>{
    ...
    contactInfo.isAllowed = foundContact[0].isAllowed
    toggleModal();
}
...
<template>
  ...
  <div class="p-3">
    <h2 class="uppercase dark:text-white text-gray-900 font-bold">Comments to confirm</h2>
            ...
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <form >
                    <div class="grid gap-4 mb-4 sm:grid-cols-1">
                        <div>
                            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">isAllowed</label>
                            <select v-model="contactInfo.isAllowed">
                ...
                <table class="w-full text-sm table-auto">
                    ...
                    <tbody class="">
                        <tr v-for="(el, index) in computedList" :key="el.id" class="border-b dark:border-gray-700">
                            <td class="px-4 py-3">{{ el.content }}</td>
                            <td class="px-4 py-3">{{ el.post.title }}</td>
                            <td class="px-4 py-3">{{ el.username }}</td>
                            <td class="px-4 py-3">{{ el.isAllowed }}</td>
                            
                            <td class="px-4 py-3 text-[20px]">
                                <div  class="bg-white rounded  shadow flex w-[100px]" >
                                    <button @click="updateContact(el.id)" class="py-1 px-4 text-green-400  hover:bg-gray-200 "><i class='bx bx-check'></i></button>
