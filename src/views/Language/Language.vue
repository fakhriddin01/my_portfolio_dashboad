...
const addContact=(evet)=>{
    ...
    const contact = {
        ...
        level: contactInfo.level,
    }

    useLanguage.create(contact).then((res)=>{
        if(res.status == 201){
...
const modifyContact=(event)=>{
    ...
    useLanguage.update(id, contact).then((res)=>{
        if(res.status == 200){
...
const removeContact=()=>{
    ...
    console.log(id);
    useLanguage.remove(id).then((res)=>{
        if(res.status == 200){
...
const updateContact = (id)=>{
    ...
    contactInfo.link = foundContact[0].level
    toggleModal();
}
...
<template>
  ...
  <div class="p-3">
    <h2 class="uppercase dark:text-white text-gray-900 font-bold">Languages</h2>
            ...
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                        <span v-if="!isUpdate">Add new language</span>
                ...
                <form >
                    <div class="grid gap-4 mb-4 sm:grid-cols-1">
                        <div>
                            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Language</label>
                        ...
                        <div>
                            <label for="link" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Level</label>
                            <input v-model="contactInfo.level" type="text" name="link" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="" required="">
                        </div>
                ...
                <table class="w-full text-sm table-auto">
                    ...
                    <tbody class="">
                        <tr v-for="(el, index) in computedList" :key="el.id" class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3">{{ el.level}}</td>
